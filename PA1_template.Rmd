...
**_Calculated Mean is `r format(mean(daily.steps$steps),digits=7)` and Median is `r median(daily.steps$steps)` _**
...
```{r echo=FALSE}
data.frame(maxsteps)
hour <- as.integer(floor(maxsteps[1,1]/100))
minute <- as.integer(maxsteps[1,1]-hour*100)
ampm <- ifelse(hour>= 12, "pm", "am")
```

**Observations:** _Based on steps taken pattern, the person's daily activity peaks around **`r hour`:`r minute` `r ampm`**._
...
**_Calculated Mean is `r format(mean(daily.steps.recon$steps),digits=7)` and Median is `r median(daily.steps.recon$steps)` _**
...
```{r echo=FALSE}
x1 <- filter(time.average.wday,weekday=="Weekday")
x2 <- filter(time.average.wday,weekday=="Weekend")
```
**Observations :**

_1. Weekends steps starts to grow up later hours compared to Weekdays_

_2. Average peak step in Weekdays `r format(max(x1$steps),digits=5)` is more than average peak step in Weekends `r format(max(x2$steps),digits=5)`_

_3. Weekend Median `r format(median(x2$steps),digits=4)` is higher than Weekday Median of `r median(x1$steps)`_